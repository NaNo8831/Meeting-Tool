...
function AgendaOutcomeWorkflowPreview() {
  return (
    <div
      className="rounded-2xl border border-dashed border-blue-200 bg-blue-50/50 p-4"
      aria-label="Proposed agenda outcome workflow preview"
    >
      <p className="text-xs font-semibold uppercase tracking-wide text-blue-700">
        Proposed workflow preview
      </p>
      <p className="mt-1 text-xs text-slate-600">
        Visual mock only — not yet functional. Pending workflow approval before
        persistence changes.
      </p>
      <div className="mt-3 space-y-3 rounded-xl border border-blue-100 bg-white p-3 opacity-80">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          <span className="rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 font-semibold text-slate-500">
...
function DecisionsRollupPreview() {
  return (
    <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600">
      <span className="font-semibold text-slate-700">Future direction:</span>{' '}
      This section becomes a rollup summary from agenda outcomes, with optional
      standalone add for items outside an agenda item.
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        ...
        {section.id === 'agenda' && !isReadOnly ? (
          <AgendaOutcomeWorkflowPreview />
        ) : null}
        {section.id === 'decision' && !isReadOnly ? (
          <DecisionsRollupPreview />
        ) : null}
      </div>
