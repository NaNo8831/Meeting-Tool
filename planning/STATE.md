...
- Current focus: Phase 3 **PR 2A dashboard query/access abstraction** centralizes `/dashboard` cloud meeting listing/classification in `app/lib/dashboardMeetings.ts` while preserving the existing flat dashboard visuals. Visible Supabase RLS rows are classified as owned when `meeting.owner_id === auth.user.id`; non-owned visible rows are treated as shared, with owner-only lifecycle actions withheld. Duplicate and archived soft-delete now use ownership-safe Supabase requests under membership-aware RLS: duplicate inserts avoid representation-return RLS coupling, and archived soft-delete uses a narrow owner-only RPC because the regular `meetings` update policy can reject the new soft-deleted row once `deleted_at` makes it non-active.
...
- Continue Phase 3 after the shared-access foundation with PR 2 dashboard discovery/listing/entry. PR 2A introduces the dashboard query/access abstraction; PR 2B should add the visible Owned by Me / Shared with Me sections, search across both sections, and alphabetical sorting within each section.
...
- Next recommended action: implement PR 2B as the visible sectioned dashboard UI on top of the PR 2A `DashboardMeeting` abstraction, without adding invite UI, member management, role editing, ownership transfer, Viewer UX, RLS changes, migrations, auth changes, Local Mode changes, autosave expansion, or realtime collaboration.
...
- Next recommended action: **PR 2B — Owned by Me / Shared with Me UI** should consume the new `DashboardMeeting` classification to render separate owned/shared sections, search across both, and sort alphabetically within each section.
