...
- Objective card polish keeps the shared distribution rules while using the shared layered vertical color menu above wrapped rows without click-through, placing Open details beside the task summary, opening newly added Defining Objectives directly into workflow details, and allowing Standard Operating Objective titles to wrap to two lines.
