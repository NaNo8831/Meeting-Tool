...
- Current focus: Phase 3 **PR 2A dashboard query/access abstraction** now centralizes `/dashboard` cloud meeting listing/classification in `app/lib/dashboardMeetings.ts` while preserving the existing flat dashboard visuals. Visible Supabase RLS rows are classified as owned when `meeting.owner_id === auth.user.id`; non-owned visible rows are treated as shared, with owner-only lifecycle actions withheld.
