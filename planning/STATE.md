...
- **Sprint 3 — Autosave Resilience / Rescue Paths (`fix/autosave-resilience`, 2026-07-29):** Sprint 2's audit found the token fix addressed the trigger but not the underlying fragility: the app had no safety net if a write ever failed, for any reason. This sprint hardens the two unprotected rescue paths. **Gap A — Manual Save retry:** `saveWorkspaceData` (`app/lib/supabaseClient.ts`) now throws `SupabaseRequestError` instead of a plain `Error`, matching the five autosave write functions. `runAutosaveWrite` (`app/hooks/useWorkspacePersistence.ts`) is now exported and reused — not duplicated — by Manual Save's call site (`saveWorkspaceBackupToCloud` in `MeetingWorkspace.tsx`): a Manual Save rejected for an expired token (401) is retried once against a renewed token; an RLS denial (403) or any other failure still fails fast and visibly; a 401 during deliberate sign-out neither retries nor surfaces an error (added the same `isSigningOutRef` guard the autosave surfaces already use to `handleSaveCloudMeeting`'s catch block). The Manual Save button's visible states are unchanged. **Gap B — reload recovery banner:** immediately before `handleLoadCloudMeeting` overwrites the browser's cached copy of the workspace with the server's, the nine `useLocalStorage`/`useObjectives`-mirrored keys (`leadership-objectives`, `leadership-meetings`, `leadership-active-meeting-id`, `leadership-dashboard-title`, `leadership-organization-info`, `leadership-meeting-setup-completed`, `leadership-meeting-section-order`, `leadership-strategic-topic-items`, `leadership-standard-operating-objectives`) are read directly from scoped `localStorage` (not React state, to avoid the async hydration race) and compared to the incoming server signature via `getWorkspaceStorageSignature`, gated by `hasMeaningfulWorkspaceStorage`. A meaningful difference is preserved in a new per-workspace `recovery-snapshot` localStorage key the overwrite never touches, and a small dismissible amber banner (reusing the app's existing caution-banner visual language) offers to restore it via the existing `applyWorkspaceBackupToState` path; dismissing just clears the slot. The load itself is never blocked or delayed. No timestamp comparison or "newer wins" logic anywhere — see `planning/DECISIONS.md` (2026-07-29) for why that was explicitly rejected. Strategic Topic Notes are out of scope for the Gap B snapshot (not `useLocalStorage`-mirrored). No schema/migration/RLS change; no `MeetingWorkspace.tsx` refactor beyond these two call sites; no change to the five already-hardened autosave surfaces, Backup/Restore, or export/import. **Validation:** `npx tsc --noEmit` clean, `npm run build` succeeds, `npm run lint` shows only the same 5 pre-existing errors in the untracked 120x `scripts/` folder (unchanged from Sprint 2). **Pending before merge to `dev`:** sign-in-gated manual testing (Manual Save retry, the reload/recovery banner, sign-out/in, and the acceptance §6 end-to-end reproduction of the 2026-07-15 scenario) and Vercel preview validation — these require Supabase auth credentials the Builder does not have and cannot enter on the Project Lead's behalf; handed to the Project Lead to run locally or on a Vercel preview.
- **Sprint 2 — Autosave Session Expiry (`fix/autosave-session-expiry`, 2026-07-19):** A beta user lost a live meeting's work; reported Blocking. Confirmed cause: the Supabase access token expires after ~1h and nothing renewed it while the tab stayed open (the refresh check ran once, on mount), so every autosave write past the hour was rejected with a JWT error and the reload used to clear the error discarded the uncommitted edits. **Step 1** renews the token from its own `expiresAt` on a self-rescheduling timer and on `visibilitychange`, single-flight across the three `useSupabaseAuth` instances, signing out only when the refresh token is genuinely rejected. **Step 2** retries a write rejected for a stale token once against a fresh token (401 only; RLS 403 still fails fast) and logs status/body/remaining-lifetime on failure. Added `SupabaseRequestError` to carry HTTP status; converted only the refresh/getUser and the 13 autosave-write throw sites. No schema/migration/RLS change; no `MeetingWorkspace.tsx` refactor; Manual Save, export/import, autosave debounce and per-surface status unchanged. **Validation:** `tsc --noEmit` clean, `build` succeeds, `lint` shows only the 5 pre-existing errors in the untracked 120x `scripts/` folder; §1 (diagnosis), §3 (retry, live Test B — `401 PGRST301` rejected → renewed → retried → saved, no data lost), §2 (renewal), and **§6 (long-meeting — the criterion that matters)** confirmed on a local shortened-expiry run (2026-07-19): 2 renewals fired, both 200, one per cycle (single-flight + reschedule), no autosave error, and edits made 5 and 10 min post-login survived reload. **Pending before main:** §5 flows (task/rich-text/Backup + sign-out/in), optional §2 tab-wake/timer checks, and a §8 Vercel preview smoke, then `dev` → `main`. **Audit finding:** the token fix is the root-cause fix and introduces no empty-state-overwrite path; two *resilience* gaps that turned the transient failure into permanent loss (Manual Save has no retry; a reload discards local-newer edits) are deferred to an Architect sprint — see `planning/POST_BETA_BACKLOG.md` and `planning/feedback/2026-07-18-feedback-report.md`. Three other feedback items deferred (editors/Edit Playbook = product decision; Tactical History coverage + colours; dashboard rename).
- **Sprint 1 — Line-Ending Normalization (`chore/line-ending-normalization`, 2026-07-18):** First sprint run under the 120x method. The repo had no `.gitattributes` and no `core.autocrlf` config, so the Windows/WSL machine and the Linux machine disagreed about line endings on every file — `git status` showed 135 modified files when only `package-lock.json` had actually changed. `.gitattributes` now governs line endings for the repository (`* text=auto eol=lf` plus binary exclusions), so git normalizes to LF on commit regardless of contributing machine. Committed blobs were already LF, so no renormalization commit was needed and `git blame` is fully preserved; no commit in this sprint touched more than one file. The genuine `package-lock.json` change (transitive dev dependency refresh, no direct dependency changed) landed as its own reviewable commit. `docs/MACHINE_SETUP_LINE_ENDINGS.md` gives the Project Lead the re-clone and git-config steps for the Windows/WSL machine. No app code, schema, migration, RLS, auth, persistence, or UI was touched — zero files under `app/`, `data/`, or `supabase/`. **Merged to `dev` 2026-07-18** (merge commit `30a5c88`, `--no-ff`; the merge commit message carries the full change/validation record since the `gh` CLI was unavailable to open a GitHub PR). Remaining: the Windows/WSL machine still needs the steps in `docs/MACHINE_SETUP_LINE_ENDINGS.md` — the repository is protected regardless, but that machine writes CRLF locally until then.
- **Sprint 5 — Editable Surface Standardization + Layout Uniformity (`sprint-5-editable-surfaces`):** All edit-existing-content surfaces converted to single-click inline, blur-save, change-guarded. `EditableField` and `RichTextEditor` standardized. Rich text toolbar visibility split into always-visible (Playbook, DO/Task/SOO, Setup) and discoverable (agenda notes) buckets. Yellow hover highlight applied to all click-to-edit surfaces. RichTextEditor toolbar active-state scoped to focused editor instance. DO, SOO, and Task detail modals unified to identical structure (eyebrow, labels, `EditableField` title, always-editing description). SOO converted from draft-modal/Save-button to blur-save. Build passing. **Merged to `dev` 2026-06-23 (merge `62d6eff`) and shipped to `main`/production. The local/remote `sprint-5-editable-surfaces` branch was a stale pointer at the pre-merge tip and was deleted 2026-07-19; the work lives on in `dev`/`main`.**
- **Header sprint (`sprint-pre-beta-header-v2`):** MeetingHeader redesigned into five zones. Simplified three-button lifecycle model (End Meeting / View / Start Meeting). Two-chip model (Open / Closed / Test Mode). One-meeting-per-date enforcement restored and updated to respect Test Mode date override. Test Mode chip shows open/closed sub-state and known limitation note in the help popover. Phantom meeting fix, agenda items FK fix, and Manual Save inline status complete. **Merged to `dev` (merge `39dc152`) and shipped to `main`/production (Beta launch, `558113a`).**
