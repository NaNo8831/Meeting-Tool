...
| Should PR 2A retrieve membership role from `meeting_members`, or is `owner_id === auth.user.id` sufficient for first Owned by Me / Shared with Me classification? | Phase 3 dashboard architecture | Refined: PR 2A keeps role retrieval minimal because `meeting_members` remains access-management data; dashboard classification uses visible `meetings` rows from RLS plus `meeting.owner_id === auth.user.id`, setting owned rows to role `owner` and leaving shared row role unknown for PR 2B. |
| Should archived Shared with Me cards remain openable when archive visibility is enabled, matching current archived owned-card behavior? | Phase 3 dashboard UX | Answered for PR 2: archived shared meetings follow the same archive visibility toggle as owned meetings and remain Open-only when shown. |
